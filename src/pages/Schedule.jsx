...
const Schedule = () => {
    ...
    const [formData, setFormData] = useState({
        customerId: '',
        schedules: [
            { category: '', posterIds: [], date: '', posters: [] },
        ],
    });
    ...
    const togglePosterSelection = (index, posterId) => {
        const newSchedules = [...formData.schedules];
        const posterIds = newSchedules[index].posterIds;
        if (posterIds.includes(posterId)) {
            newSchedules[index].posterIds = posterIds.filter(id => id !== posterId);
        } else {
            newSchedules[index].posterIds = [...posterIds, posterId];
        }
        setFormData(prev => ({
            ...prev,
            schedules: newSchedules,
    ...
    const addScheduleRow = () => {
        setFormData(prev => ({
            ...prev,
            schedules: [...prev.schedules, { category: '', posterIds: [], date: '', posters: [] }],
        }));
    };
    ...
    const validateForm = () => {
        ...
        for (let i = 0; i < formData.schedules.length; i++) {
            ...
            if (!sched.category || sched.posterIds.length === 0 || !sched.date) {
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        {formData.schedules.map((sched, index) => (
                            <div key={index} className='border rounded p-3 mb-3'>
                                ...
                                {sched.category && (
                                    <div className='mb-3'>
                                        <label className='form-label'>Posters</label>
                                        {sched.posters.length > 0 ? (
                                            <div className='row'>
                                                {sched.posters.map(poster => {
                                                    ...
                                                    const isSelected = sched.posterIds.includes(poster._id);
                                                    return (
                                                        <div key={poster._id} className='col-md-4 mb-3'>
                                                            <div
                                                                ...
                                                                onClick={() =>
                                                                    togglePosterSelection(index, poster._id)
                                                                }>
                                                                <img
                                                                    src={imageUrl}
